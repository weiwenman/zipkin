...
final class DependencyLinkV2SpanIterator implements Iterator<Span> {
  ...
  static final class ByTraceId implements Iterator<Iterator<Span>> {
    ...
    public Iterator<Span> next() {
      ...
      currentTraceIdHi = hasTraceIdHigh ? Long.parseLong(traceIdHigh(delegate)) : 0L;
      currentTraceIdLo = Long.parseLong(delegate.peek().getValue(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID));
  ...
  public boolean hasNext() {
    return delegate.hasNext()
      ...
      && Long.parseLong(delegate.peek().getValue(ZipkinSpans.ZIPKIN_SPANS.TRACE_ID)) == traceIdLo;
  }
  ...
  public Span next() {
    ...
    String spanId = row.getValue(ZipkinSpans.ZIPKIN_SPANS.ID);
    boolean error = false;
    ...
    while (hasNext()) { // there are more values for this trace
      ...
      String value = emptyToNull(next, ZIPKIN_ANNOTATIONS.SERVICE_NAME);
    ...
    String parentId = maybeGet(row, ZipkinSpans.ZIPKIN_SPANS.PARENT_ID, "0");
  ...
  static String traceIdHigh(PeekingIterator<Record> delegate) {
    return "0";
  }

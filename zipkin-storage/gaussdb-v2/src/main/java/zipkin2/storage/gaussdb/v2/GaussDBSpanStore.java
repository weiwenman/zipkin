...
final class GaussDBSpanStore implements SpanStore, Traces, ServiceAndSpanNames {
  ...
  GaussDBSpanStore(GaussDBStorage storage, Schema schema) {
  ...
  public Call<List<Span>> getTrace(String traceId) {
    return dataSourceCallFactory.create(selectFromSpansAndAnnotationsFactory.create(traceId));
  }
  ...
  public Call<List<List<Span>>> getTraces(Iterable<String> traceIds) {
    ...
    for (String traceId : traceIds) {
      ...
      traceIdPairs.add(new Pair(
          hexTraceId.length() == 32 ? hexTraceId.substring(0, 16) : "0", hexTraceId
        )
      );
    }
